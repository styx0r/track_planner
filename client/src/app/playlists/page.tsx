...
export default function PlaylistsPage() {
  ...
  // App mode: performance (safe) or rehearsal (full controls)
  const [mode, setMode] = useState<AppMode>('rehearsal');
  const isPerformanceMode = mode === 'performance';
  
  // Count-in settings (only used in rehearsal mode)
  ...
  // In performance mode, no count-in
    ...
    <div className={`${styles.page} ${isPerformanceMode ? styles.performanceMode : ''}`}>
      ...
      <main className={styles.main}>
        ...
        <aside className={styles.sidebar}>
          ...
          {/* Sync Debug Info - only in rehearsal mode */}
