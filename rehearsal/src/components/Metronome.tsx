...
export function Metronome({
  ...
  // Audio settings (only used in rehearsal mode)
  ...
  // Auto-initialize audio when metronome becomes enabled (only in rehearsal mode)
  useEffect(() => {
    if (!performanceMode && enabled && soundType !== 'off' && !audioInitialized) {
      initAudioContext();
      setAudioInitialized(true);
    }
  }, [enabled, soundType, audioInitialized, performanceMode]);

  // Initialize audio on first user interaction
  const handleEnableClick = useCallback(() => {
    if (!performanceMode && !audioInitialized && soundType !== 'off') {
      initAudioContext();
      setAudioInitialized(true);
    }
    onToggle(!enabled);
  }, [audioInitialized, soundType, enabled, onToggle, performanceMode]);
  ...
  const animate = useCallback(() => {
    ...
    if (beat !== lastBeatRef.current) {
      ...
      // Play regular beat sound (only in rehearsal mode)
  ...
  return (
    <div className={`${styles.metronome} ${performanceMode ? styles.performanceMode : ''}`}>
      ...
      {/* BPM Display - always shown, but read-only in performance mode */}
      <div className={styles.bpmDisplay}>
        <span className={styles.bpmValue}>{bpm}</span>
        <span className={styles.bpmUnit}>BPM</span>
      </div>

      {/* Controls - only shown in rehearsal mode */}
      {!performanceMode && (
        <>
